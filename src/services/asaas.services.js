...
async function criarClienteAsaas(dadosCliente) {
    ...
    data: {
        ...
        notificationDisabled: true,
        groupName: 'Grupo BALCÃO E BANDEJA',
        company: 'BALCÃO E BANDEJA'
    }
    };
...
async function cobrancaPixAsaas(dadosCliente) {
    const options = {
        ...
        data: {
            ...
            externalReference: dadosCliente.externalReference, // ID do pedido ou identificador
            customFields: [
                { name: "endereco", value: JSON.stringify(dadosCliente.endereco || {}) },
                { name: "frete", value: JSON.stringify(dadosCliente.frete || {}) },
                { name: "itens", value: JSON.stringify(dadosCliente.itens || []) }
            ]
        }
    };
...
async function cobrancaCartaoAsaas(dadosCliente) {
  const options = {
    ...
    data: {
      ...
      description: "Pedido pago com Cartão - BALCÃO E BANDEJA",
      remoteIp: "10.0.0.118",
      customer: dadosCliente.customer,
      creditCard: {
